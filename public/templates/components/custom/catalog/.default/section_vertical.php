...
<article>
    ...
    $sectionCount = CIBlockSection::GetCount(array('SECTION_ID' => $arResult['VARIABLES']['SECTION_ID']));
    $showOnFilter = false;
    if (empty($arSection['IBLOCK_SECTION_ID']) && $sectionCount > 0) {
        //$isFilter = false;
	...
	<? $count = getProductListCount();
		...
		[$sortField, $sortOrder]=explode('_', $sort);
		if(!$sortField) $sortField='sort';
		else {
			if ($sortField == 'price') {
			    $productId = CIBlockElement::GetList(array(), array('IBLOCK_ID' => 1, 'ACTIVE' => 'Y'), false, false, array('ID'))->Fetch()['ID'];
