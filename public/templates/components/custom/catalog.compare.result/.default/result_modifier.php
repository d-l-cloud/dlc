...
foreach($arResult["ITEMS"] as $arElement) { 
	array_push($arTmp, $arElement["ID"]);
} 

$arResult["MEASURES"] = \Bitrix\Catalog\ProductTable::getCurrentRatioWithMeasure($arTmp);

if (count($arTmp)) {
	$res = CIBlockElement::GetList(Array(), array("ID" => $arTmp), false, false, array("ID", "PREVIEW_PICTURE"));

	while($ar_fields = $res->GetNext()) {
		$arImgs[$ar_fields["ID"]] = CFile::GetPath($ar_fields["PREVIEW_PICTURE"]);
	}	
}

foreach($arResult["ITEMS"] as $key => &$arItem){
	if (isset($arImgs[$arItem["ID"]]))
		$arResult["ITEMS"][$key]["PREVIEW_PICTURE"] = $arImgs[$arItem["ID"]];
}