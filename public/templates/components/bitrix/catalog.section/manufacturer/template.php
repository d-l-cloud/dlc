...
<div class="catalog-slider-row">
    <?php
    $url = $arResult['SECTION_PAGE_URL'];
    if (!empty($arParams['BRAND_LINK_FILTER'])) {
        $url .= $arParams['BRAND_LINK_FILTER'];
    }

    $name = $arResult['NAME'];
    if (!empty($arParams['BRAND_CATEGORY_NAME'])) {
        $name = $arParams['BRAND_CATEGORY_NAME'];
    }
    ?>
	<a href="<?= $url ?>">
		<h2 class="slider-header"><?= $name ?></h2>
	</a>
